...
<div class="container  py-4">
    <div class="alert alert-primary h1 text-center"
        role="alert"
        id="alert"
        style="display: none;">
    </div>
                ...
                <div style="overflow-x:auto;"
                    ...
                    <div class="d-flex flex-row flex-nowrap">
                        <?php foreach ( $profile as $index => $P ): ?>
                            <div class="card col card-block mx-2 bg-light shadow"
                                style="min-width: 300px;">
                                <h5 class="card-header text-dark"
                                    style="background-color: <?= $P[ 'color' ]; ?>;"><?= $MikroTik[ 'currency' ] . $P[ 'price' ]; ?>
                                </h5>
                                <div class="card-body">
                                    <p class="card-text mb-0">
                                        Duration: <span class="text-primary fw-bold">
                                            <?= secondsToWords( $P[ 'duration' ] ); ?>
                                        </span><br>
                                        Validity: <span class="text-primary fw-bold">
                                            <?= secondsToWords( $P[ 'validity' ] ); ?>
                                        </span><small class="text-secondary"> ( Pausable )</small><br>
                                        Data: <span class="text-primary fw-bold">
                                            <?= $P[ 'data' ] == 0 ? 'Unlimited' : byteFormat( $P[ 'data' ], "d", 0 ) ?>
                                        </span><br>
                                        Server: <span class="text-primary fw-bold">
                                            <?= $P[ 'server' ]; ?>
                                        </span><br>
                                        Profile: <span class="text-primary fw-bold">
                                            <?= $P[ 'profile' ]; ?>
                                        </span><br>
                                    </p>
                                    <div class="d-flex gap-2">
                                        <button href="#inputForm"
                                            class="col btn btn-primary"
                                            onclick="profile('<?= $P[ 'id' ]; ?>','select')">Select <?= $MikroTik[ 'currency' ] . $P[ 'price' ]; ?></button>
                                        <button class="col-2 btn btn-danger"
                                            onclick="deleteProfile('<?= $P[ 'id' ]; ?>','remove')"
        ...
        <div class="card-body">
            ...
            <form id="inputForm"
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-3">
                        ...
                        <div class="col  position-relative">
                            <select class="form-select"
                                ...
                                <option value="true">Yes ( Voucher is disabled on generate )</option>
                                <option value="false">No ( Voucher is active on generate )</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-6  col-md-6  col-lg-3">
                ...
                <div class="row">
                    <div class="col">
                        <label class="form-label fw-bold"
                            for="Price">Voucher Price</label>
                        <div class="row">
                            <div class="col">
                                <input class="form-control Clear-all "
                                    type="number"
                                    name="price"
                                    value=""
                                    id="price"
                ...
                <div class="row gap-2 m-2 ">
                    <button class="col-12 btn btn-success"
                        type="submit"
                        id="generate"><i class="fa fa-save"></i> Generate</button>
                </div>
            </form>
            <div class="row gap-2 m-1">
                <button class="col-12 col-sm col-lg btn btn-danger"
                    id="reset"><i class="fas fa-trash"></i> Clear values</button>
                <button class="col-12 col-sm col-lg btn btn-primary"
                    id="addTemp"><i class="fas fa-plus"></i> Add to template</button>
                <button class="col-12 col-sm col-lg  btn btn-warning  text-white text-white"
                    style="display: none;"
                    id="printbtn"
                    type="button"
                    data-bs-toggle="modal"
                    data-bs-target="#print-modal"><i class="fas fa-print"></i> View Print Page</button>
            </div>
        </div>
    </div>
...
<div class="modal fade"
    ...
    tabindex="-1"
    aria-labelledby="print-modalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content bg-light">
            <div class="modal-header">
                <h1 class="modal-title fs-5"
                    id="print-modalLabel">Print</h1>
                <button type="button"
                    class="btn-close"
            ...
            <div class="modal-body"
                style="color: #000;">
                <link rel="stylesheet"
                    href="../src/voucher.css?v=<?php echo rand(); ?>">
                <div id="printPage">
                </div>
            </div>
            <div class="modal-footer">
                <div class=" row gap-2">
                    <button class="col col-sm col-lg btn btn-danger"
                        type="button"
                        id="clearpage"><i class="fas fa-trash"></i> Clear Print Page</button>
                    <button class="col col-sm col-lg btn btn-success"
                        type="button"
                        data-bs-dismiss="modal"><i class="fa fa-save"></i> Generate More</button>
                    <button class="col col-sm col-lg btn btn-warning text-white"
                        type="button"
                        id="printvoucher"><i class="fas fa-print"></i> Print</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    if ("" === e.textContent.trim());
        ...
        n.document.write("<link href=src/voucher.css?v=<?php echo rand(); ?> rel=stylesheet>");
        setTimeout(function () {
            n.document.write("<html><body><title>" + t + "</title>" + o + "</body></html>");
            n.print();
        }, 100)
    }
})
